...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    document.addEventListener("click", (e)=>{
        if (!e.target.closest(".header__burger") && !e.target.closest(".navigation")) {
            navigation.classList.remove("active");
            burgerBtn.classList.remove("active");
        }
    });
    ...
    const thisSlide = (index)=>{
        for (let dot of dots)dot.classList.remove("dot-active");
...
const toggleProfileMenu = ()=>{
    ...
    document.addEventListener("click", (e)=>{
        if (!e.target.closest(".profile__icon") && !e.target.closest(".profile__menu")) profileMenu.classList.remove("active");
    });
};
toggleProfileMenu();
// CLOSE MODAL
...
const register = (e)=>{
    ...
    let exist = users.length && JSON.parse(localStorage.getItem("users")).some((data)=>data.firstName.toLowerCase() == firstName.toLowerCase() && data.lastName.toLowerCase() == lastName.toLowerCase());
    if (!exist) {
        users.push({
            firstName,
            lastName,
...
const logIn = (e)=>{
    ...
    if (currentUser) {
        if (password == currentUser.passwordRegister) {
            ...
            loginForm.reset();
            loggedInState();
            profileIcon.innerHTML = `${currentUser.firstName[0]}${currentUser.lastName[0]}`.toUpperCase();
...
},{}]},["gEwwu","1SICI"], "1SICI", "parcelRequire04be")
